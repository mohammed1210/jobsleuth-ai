  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.auth.signInWithOtp({
        email,
        options: {
          emailRedirectTo: getMagicLinkRedirect(),
        },
      });
      if (error) {
        ...
        {sent ? (
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="text-left">
                  ...
                  <ol className="text-sm text-brand-800 space-y-1 list-decimal list-inside">
                    ...
                    <li>You'll return to JobSleuth automatically — no password needed.</li>
                  </ol>
                </div>
              </div>
            </div>
          ...
          <div className="card p-10">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600">Sign in with magic link - no password required</p>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-gray-200">
              <p className="text-sm text-gray-600 leading-relaxed">
                <strong className="text-gray-900">No password needed.</strong> We'll send you a secure one-time link by email.
              </p>
            </div>

            <div className="mt-6 text-center">
              <Link href="/" className="text-sm text-brand-600 hover:text-brand-700 font-medium">← Back to Home</Link>
            </div>
          </div>
        )}
      </main>
