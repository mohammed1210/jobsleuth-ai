...
function hasOptionalAtsDocumentField(text: string, documentPattern: string): boolean {
  ...
  const optionalField = new RegExp(`^(?:${documentPattern})\\s*import type { ApplicationType } from '@/lib/applicationBuilderApi';
...
function hasNegatedDocumentInstruction(text: string, documentPattern: string): boolean {
  ...
  // keep it as required.
, 'i');
  const requiredField = new RegExp(`^(?:${documentPattern})\\s*\\*\\s*import type { ApplicationType } from '@/lib/applicationBuilderApi';
