    ...
    <section className="card p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-xl font-bold text-gray-900">Build the application</h2>
          <p className="mt-1 max-w-2xl text-sm text-gray-500">Draft from evidence JobSleuth can support. Settings and audit detail stay out of the way unless you need them.</p>
        </div>
        {selectedPart && (
          <span className="rounded-full bg-brand-50 px-3 py-1 text-xs font-semibold text-brand-700">
            {selectedPart.label}{selectedPart.wordLimit ? ` · ${selectedPart.wordLimit} words` : ''}
          </span>
        )}
      </div>
      ...
      {selectedPart?.kind === 'behaviour' && (
        <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-900">
          <strong>Behaviour component:</strong> only Evidence Bank examples explicitly tagged <strong>{selectedPart.behaviourName}</strong> will be used.
        </div>
      )}
      ...
      {readiness.needsStrengthening ? (
        <div className="mt-4 rounded-xl border border-amber-300 bg-amber-50 p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {!draftAnyway ? (
              <button type="button" onClick={() => setDraftAnyway(true)} className="btn-ghost text-sm">Draft with current evidence</button>
            ) : (
              <span className="inline-flex items-center rounded-lg border border-amber-300 bg-white px-3 py-2 text-xs font-semibold text-amber-900">Current-evidence drafting enabled</span>
            )}
          </div>
        </div>
      ) : (
        <div className="mt-4 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-900">
          <strong>Ready to draft.</strong> All {readiness.total} essential criteria have Strong support.
        </div>
      )}
      ...
      <details className="mt-4 rounded-xl border bg-white">
        <summary className="cursor-pointer list-none px-4 py-3 text-sm font-semibold text-gray-800">
          Draft settings
          <span className="ml-2 text-xs font-normal text-gray-500">Role, organisation, format and word limit</span>
        </summary>
        <div className="space-y-4 border-t p-4">
          <div className="rounded-xl bg-gray-50 p-4">
            ...
            {instructions.requiredDocuments.length > 0 && (
              <p className="mt-2 text-sm text-gray-600">Required: {instructions.requiredDocuments.join(' + ')}</p>
            )}
          </div>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3">
        <button type="button" onClick={build} disabled={building || (readiness.needsStrengthening && !draftAnyway)} className="btn-primary disabled:cursor-not-allowed disabled:opacity-50">
          {building ? 'Building evidence-backed draft…' : `Build ${selectedPart?.label ?? 'application'} draft`}
        </button>
        <span className="text-xs text-gray-500">{wordLimit} word limit</span>
      </div>
        ...
        <div className="mt-6 space-y-5 border-t pt-6">
          {result.warnings.length > 0 && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
              ...
              <ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-amber-900">{result.warnings.map((w) => <li key={w}>{w}</li>)}</ul>
            </div>
          )}
          ...
          <div>
            <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="font-semibold text-gray-900">{selectedPart?.label ?? 'Editable'} draft</h3>
                <p className="text-xs text-gray-500">Edit freely, then check the facts before submitting.</p>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-sm text-gray-600">{liveWordCount} / {wordLimit} words</span>
                <button type="button" onClick={copyDraft} className="btn-secondary px-4 py-2 text-sm">Copy</button>
              </div>
            </div>
            <textarea value={draft} onChange={(e) => setDraft(e.target.value)} className="min-h-[22rem] w-full rounded-xl border px-4 py-3 leading-7" placeholder="A supported draft will appear here." />
          </div>
          ...
          <details className="rounded-xl border bg-white">
            <summary className="cursor-pointer list-none px-4 py-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-semibold text-gray-900">Draft checks</p>
                  <p className="text-sm text-gray-500">Criteria coverage and paragraph-level evidence audit</p>
                </div>
                <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-600">Optional review</span>
              </div>
            </summary>
                  ...
                  {result.coverage.map((item, index) => (
                    <div key={`${item.requirement}-${index}`} className="rounded-xl border p-3">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <span className="rounded-full bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-700">{statusLabel[item.status] || item.status}</span>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <div className="mt-3 space-y-3">
                    {result.paragraphs.map((paragraph, index) => (
                      <details key={`${paragraph.evidence_ids.join('-')}-${index}`} className="rounded-xl border p-4">
                        <summary className="cursor-pointer text-sm font-semibold text-gray-900">Paragraph {index + 1} · {paragraph.grounding_status}</summary>
          ...
          <details className="rounded-xl border border-brand-100 bg-brand-50/30">
            <summary className="cursor-pointer list-none px-4 py-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="font-semibold text-gray-900">Tester feedback</p>
                  <p className="text-sm text-gray-600">About 1 minute · ratings only, no vacancy or draft text stored</p>
                </div>
                <span className="rounded-full bg-white px-3 py-1 text-xs font-semibold text-brand-700">{feedbackSaved ? 'Saved' : 'Pilot'}</span>
              </div>
            </summary>
            ...
            <div className="space-y-4 border-t border-brand-100 p-4">
              ...
              <button type="button" disabled={savingFeedback || feedbackSaved || !isPilotFeedbackComplete(pilotFeedback)} onClick={submitFeedback} className="btn-secondary disabled:opacity-60">
                {feedbackSaved ? 'Feedback saved' : savingFeedback ? 'Saving…' : 'Save tester feedback'}
              </button>
            </div>
          </details>
        </div>
