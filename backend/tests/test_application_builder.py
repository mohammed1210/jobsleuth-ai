...
def test_cover_letter_fallback_respects_minimum_word_limit(monkeypatch):
    monkeypatch.setattr("routes.application_builder.semantic_application_draft", lambda *args, **kwargs: (None, "no_api_key"))
    card = evidence_card().model_copy(
        update={
            "task": " ".join(["grounded"] * 120),
            "actions": [],
            "outcome": "The work was completed safely.",
        }
    )
    payload = {
